...
export const Cart: React.FC = () => {
  const { isCartOpen, toggleCart, language } = useUIStore();
  ...
  const handleCheckout = () => {
    if (!user) {
        ...
        // Here we would open the CheckoutModal
    }
  };
        ...
        <>
          ...
          >
            ...
            <div className="p-6 border-b border-gray-200 dark:border-white/10 flex justify-between items-center">
              <h2 className="font-serif text-2xl font-bold text-primary dark:text-white">Cart ({items.length})</h2>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-gray-400 space-y-4">
                    ...
                    <button onClick={toggleCart} className="text-accent hover:underline">Start Shopping</button>
                </div>
              ) : (
                items.map((item) => (
                  <motion.div 
